...
public class InnerView extends RelativeLayout {
	...
	public InnerView(Context context, AttributeSet attrs, int defStyle){
		super(context,attrs,defStyle);
	}

	/** 初始化视图 */
	private void initView() {
		View.inflate(context, R.layout.homepage_pics_view, this);

		mViewPager = (ViewPager) findViewById(R.id.vp_pager);
		mPointContainer = (LinearLayout) findViewById(R.id.point_container);
		mTvTitle = (TextView) findViewById(R.id.tv_vp_title);
		mLl = (LinearLayout) findViewById(R.id.ll_viewpager);
	}
	...
	private void initData() {
		...
		if (mListDatas != null) {
			for (int i = 0; i < mListDatas.size(); i++) {
				...
				point.setBackgroundResource(R.mipmap.point_normal);
